...
type Post = { id: number; category: string; region: string; title: string; content: string; nickname: string; createdAt: string; commentCount: number };
...
const categories: readonly Category[] = COMMUNITY_CATEGORIES;
const regions: readonly Region[] = COMMUNITY_REGIONS;

const categoryMap = new Map(categories.map((category) => [category.id, category]));
const regionMap = new Map(regions.map((region) => [region.id, region]));
const regionRequiredCategories = new Set(["players", "recruit", "review", "manners", "manager"]);
...
export default function CommunityPanel() {
  ...
  const loadPosts = useCallback(async (category = selectedCategory, region = selectedRegion) => {
    setLoading(true);
    setError("");
    try {
      const response = await fetch(`/api/community?category=${encodeURIComponent(category)}&region=${encodeURIComponent(region)}`, { cache: "no-store" });
  ...
  async function submitPost(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setPostForm({ category: postForm.category, region: postForm.region, title: "", content: "", nickname: "", password: "" });
      setNotice("게시글을 등록했습니다.");
      await loadPosts(selectedCategory, selectedRegion);
    ...
    <section className="community-shell" aria-live="polite">
      ...
      <div className="community-layout">
        <aside className="community-categories">
          ...
          <strong className="community-region-heading">지역</strong>
          {regions.map((region) => <button type="button" key={region.id} className={selectedRegion === region.id ? "active" : ""} onClick={() => changeRegion(region.id)}>{region.name}</button>)}
        ...
        <div className="community-content">
          ...
          {mode === "write" ? (
            ...
            <article className="community-detail">
              ...
              <div className="community-detail-head"><div><span className="community-category-label">{categoryMap.get(selectedPost.category)?.label ?? selectedPost.category}</span><span className="community-region-label">{regionMap.get(selectedPost.region)?.name ?? "전국"}</span><h3>{selectedPost.title}</h3><p>{selectedPost.nickname} · {formatDate(selectedPost.createdAt)}</p></div><button type="button" className="text-button danger" onClick={async () => { const password = window.prompt("게시글 비밀번호를 입력해주세요."); if (!password) return; const response = await fetch(`/api/community/${selectedPost.id}`, { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ password }) }); if (response.ok) { setSelectedPost(null); await loadPosts(); setNotice("게시글을 삭제했습니다."); } else { const data = await response.json(); setError(data.error ?? "삭제하지 못했습니다."); } }}>삭제</button></div>
            ...
            <>
              ...
              {detailLoading || loading ? <div className="community-empty"><span className="spinner dark" /> 게시글을 불러오는 중입니다...</div> : posts.length ? <div className="community-post-list">{posts.map((post) => <a href={`/community/${post.id}`} className="community-post-card" key={post.id}><div className="community-post-card-top"><span><span className="community-category-label">{categoryMap.get(post.category)?.label ?? post.category}</span><span className="community-region-label">{regionMap.get(post.region)?.name ?? "전국"}</span></span><span>{formatDate(post.createdAt)}</span></div><h3>{post.title}</h3><p>{post.content}</p><div><span>{post.nickname}</span><span>댓글 {post.commentCount}</span></div></a>)}</div> : <div className="community-empty"><strong>아직 게시글이 없습니다.</strong><span>첫 글을 작성해 풋살 이야기를 시작해보세요.</span><button type="button" onClick={openWriteForm}>첫 글 작성하기</button></div>}
            </>
          )}
        </div>
      </div>
